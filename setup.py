...
def get_extras_requirements() -> Optional[Dict[str, List[str]]]:
    if _is_cuda():
        return {"ray": ["ray >= 2.9"]}
    if _is_hip():
        return {"ray": ["ray == 2.9.3"]}
    if _is_neuron():
        return None
    raise ValueError("Unsupported platform, please use CUDA, ROCM or Neuron.")
...
setup(
    ...
    extras_requires=get_extras_requirements(),
