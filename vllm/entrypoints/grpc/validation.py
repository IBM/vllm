...
from vllm.entrypoints.grpc.pb.generation_pb2 import DecodingMethod, Parameters
...
class TGISValidationError(str, Enum):
    ...
    InputLength = ("input tokens ({0}) plus prefix length ({1}) plus "
                   "min_new_tokens ({2}) must be <= {3}")
    InputLength2 = "input tokens ({0}) plus prefix length ({1}) must be < {2}"
    Tokenizer = "tokenizer error {0}"
    StopSequences = ("can specify at most {0} non-empty stop sequences, each "
                     "not more than {1} UTF8 bytes")
    TokenDetail = ("must request input and/or generated tokens to request "
                   "extra token detail")
    PromptPrefix = "can't retrieve prompt prefix with id '{0}': {1}"
    SampleParametersGreedy = ("sampling parameters aren't applicable in "
...
def validate_input(sampling_params: SamplingParams, token_num: int,
                   max_model_len: int):
    ...
    if token_num + sampling_params.min_tokens > max_model_len:
        TGISValidationError.InputLength.error(token_num, 0,
...
def validate_params(params: Parameters, max_max_new_tokens: int):
    ...
    if decoding.HasField("length_penalty"):
        args = [decoding.length_penalty.start_index, decoding.length_penalty.decay_factor]
    ...
    if stopping.stop_sequences and (
        ...
        TGISValidationError.StopSequences.error(MAX_STOP_SEQS,
                                                MAX_STOP_SEQ_LENGTH)

    # Response options validation
    if resp_options.top_n_tokens > MAX_TOP_N_TOKENS:
        TGISValidationError.TopN.error(resp_options.top_n_tokens,
                                       MAX_TOP_N_TOKENS)

    if (resp_options.token_logprobs or resp_options.token_ranks
    ...
    if STRICT_PARAMETER_VALIDATION and greedy and (sampling.temperature
                                                   or sampling.top_k
                                                   or sampling.top_p
